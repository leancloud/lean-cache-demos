...
AV.Cloud.afterUpdate('Category', function(request) {
  redisClient.hsetAsync('categories', request.object.get('name'), stringifyAVObject(request.object)).catch(function(err) {
...
AV.Cloud.afterDelete('Category', function(request) {
  redisClient.hdelAsync('categories', request.object.get('name')).catch(function(err) {
    console.error(err.stack);
  });
});
